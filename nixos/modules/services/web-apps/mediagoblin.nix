...
let
  ...
  mkSubSectionKeyValue = depth: k: v: if lib.isAttrs v then let
    inherit (lib.strings) replicate;
  in (replicate depth "[") + k + (replicate depth "]") + lib.generators.toINIWithGlobalSection {
      mkKeyValue = mkSubSectionKeyValue (depth+1);
    } { globalSection = v; }
  else
    lib.generators.mkKeyValueDefault {
      mkValueString = v: if lib.isString v then ''"${v}"''
  ...
  GI_TYPELIB_PATH = let
    needsGst = (cfg.settings.mediagoblin.plugins?"mediagoblin.media_types.audio") || (cfg.settings.mediagoblin.plugins?"mediagoblin.media_types.video");
  in lib.makeSearchPathOutput "out" "lib/girepository-1.0" (with pkgs.gst_all_1; [ pkgs.glib gst-plugins-base gstreamer ]
  ...
  finalPackage = cfg.package.python.buildEnv.override {
    ...
    ++ (let inherit (cfg.settings.mediagoblin) plugins; in with cfg.package.optional-dependencies;
  ...
  config = lib.mkIf cfg.enable {
    environment.systemPackages = [
      (pkgs.writeShellScriptBin "mediagoblin-gmg" ''
        ...
        if [[ "$USER" != mediagoblin ]]; then
         sudo='exec /run/wrappers/bin/sudo -u mediagoblin --preserve-env'
        fi
        $sudo ${lib.getExe' finalPackage "gmg"} "$@"
      '')
    ];
    ...
    systemd.services = let
      serviceDefaults = {
        ...
        path = lib.optionals (cfg.settings.mediagoblin.plugins?"mediagoblin.media_types.stl") [ pkgs.blender ]
          ++ lib.optionals (cfg.settings.mediagoblin.plugins?"mediagoblin.media_types.pdf") (with pkgs; [ poppler_utils unoconv ]);
        serviceConfig = {
          AmbientCapabilities = "";
    ...
    in {
      mediagoblin-celeryd = lib.recursiveUpdate serviceDefaults {
        ...
        preStart = ''
          cp --remove-destination ${pkgs.writeText "mediagoblin.ini"
            (lib.generators.toINI { } (lib.filterAttrsRecursive (n: v: n != "plugins") cfg.settings) + "\n"
            + lib.generators.toINI { mkKeyValue = mkSubSectionKeyValue 2; } { inherit (cfg.settings.mediagoblin) plugins; })
      ...
      mediagoblin-paster = lib.recursiveUpdate serviceDefaults {
        after = [ "mediagoblin-celeryd.service" "postgresql.service" ];
        requires = [ "mediagoblin-celeryd.service" "postgresql.service" ];
        preStart = ''
